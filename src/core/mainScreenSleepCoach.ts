...
function getAwakeWhySummary(input: {
  ...
  if (input.minutesUntilNextSleep <= PREPARE_THRESHOLD_MINUTES) {
    return 'Поэтому лучше перейти на спокойное бодрствование и начать подготовку ко сну сейчас, без спешки.';
  }

  return 'Поэтому пока можно бодрствовать как обычно, а спокойный ритм оставить ближе к окну сна.';
}
